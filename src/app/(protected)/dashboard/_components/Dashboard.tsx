...
const Dashboard: React.FC = () => {
  ...
  const fetchChartData = async () => {
    try {
      ...
      console.error("Error al obtener datos para la grÃ¡fica:", error);
    }
  };
  ...
  const ProductsRunOut = async () => {
    ...
    const results = await baseService.Get<Producto>("/Productos/ProductsByRunOut");
  ...
  const APIUsersCount = async () => {
    ...
    if (results.success) {
      ...
      setCountAccess(cast.length);
    }
  };

  const [countTotalProduct, setCountTotalProduct] = useState<number>(0);
  const APITotalProducts = async () => {
    const results = await baseService.GetSimple("/VentaProducto/GetTotalProducts");
    if (results.success) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 md:mt-[-80px] mb-[-10px] gap-4 sm:py-10 px-6 pt-5 pb-[140px]">
          <div className="md:col-span-3 space-y-4">
            <div className="bg-white rounded-lg p-4 shadow-md">
              <h2 className="text-lg font-semibold mb-2">Resumen de ventas del dÃ­a</h2>
                ...
                {dbCategories.map((category, index) => (
                  <div key={index} className="flex items-center">
                    <div className="flex flex-col items-center px-10">
                      ...
                      <p className="text-gray-600">{category.nombreCategoria}</p>
                    </div>
          ...
          <div className="md:col-span-2 space-y-4">
            <div className="bg-white rounded-lg p-4 shadow-md h-[328px] flex flex-col items-center justify-center">
              <h2 className="text-lg font-semibold mb-3">Resumen de inventario</h2>
              ...
              <a href="/proveedores" className="flex flex-col items-cemter text-center">
                <img src={IconCategory} alt="CategorÃ­as" className="h-8 mt-4 mb-2" />
