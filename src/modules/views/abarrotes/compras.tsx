...
const Compras: React.FC = () => {
  ...
  const eliminarDelCarrito = useCallback((productoId: number) => {
    setCarrito((prevCarrito) =>
      CarritoService.eliminarProducto(prevCarrito, productoId),
    );
  }, []);
        ...
        <div className="flex flex-col md:flex-row justify-between p-4 bg-gray-100 min-h-screen">
          <div className="w-full md:w-2/3 bg-white p-6 rounded-lg shadow-md mb-4 md:mb-0">
            <h1 className="text-xl mb-4">
              <span className="font-semibold">Venta: </span> {numeroVenta}
            </h1>
            <p className="mb-2">
              <span className="font-semibold">Usuario: </span> {usuario}
            </p>
            <p className="mb-6 text-xl text-end">
                  ...
                  {carrito.length === 0 ? (
                    ...
                    carrito.map((item, index) => (
                      <tr
                        ...
                        className={
                          productoSeleccionadoId === item.producto.id
                            ? "bg-blue-100"
                            : ""
                        }
                        onClick={() =>
                          handleSeleccionarProducto(item.producto.id)
                        }
                      >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 p-8 md:grid-cols-3 gap-4 overflow-y-auto custom-scrollbar max-h-[600px]">
              {productosFiltrados.map((producto) => (
                ...
                >
                  ...
                  <p className="text-center mt-2 text-sm font-medium overflow-hidden text-ellipsis whitespace-nowrap max-w-full">
                    {producto.nombre}
                  </p>
