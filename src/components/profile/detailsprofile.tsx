...
const ProfileModal: React.FC<ProfileModalProps> = ({ onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updateUser = await baseService.Put(
      ...
      if (updateUser.success) {
        ...
        if (user.success) {
          UserStore.getState().setUser(id, name, email, status, rol);
            ...
            <div>
              ...
              {!passwordsMatch && (
                <p className="mt-1 text-sm text-red-600">
                  Las contraseñas no coinciden y tiene que tener 8 dígitos, 1
                  mayuscula y un caracter.{" "}
                </p>
              )}
            </div>
          </div>
