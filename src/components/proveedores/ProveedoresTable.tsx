...
import { ProveedoresTableProps } from "../../interfaces/Proveedores/ProveedoresTableProps";
...
const ProveedoresTable: React.FC<ProveedoresTableProps> = ({
  ...
  const [proveedorSeleccionado, setProveedorSeleccionado] =
    useState<IProveedores | null>(null);
  ...
  const handleConfirmDelete = async () => {
    if (proveedorAEliminar && proveedorAEliminar.id) {
      try {
        if (onProveedoresUpdate) {
          await onProveedoresUpdate();
        }
        setDeleteModalOpen(false);
        setProveedorAEliminar(null);
      } catch (error) {
          ...
          <table className="min-w-full bg-white shadow-md rounded-lg">
            <thead>
              <tr className="bg-white text-gray-700 text-center">
                <th className="p-4 text-xs sm:text-base">
            ...
            <tbody>
              {currentProveedores.map(
                (proveedor: IProveedores, index: number) => (
                  <tr
                    key={index}
